...
country_tuition <- country_tuition %>%
  ...
  select(-'Cost of One Year at University(€)', -`Tuition Fees in USD`, -`Tuition Fees in USD Thousands`) 
...
refined_country_data <- country_data %>%
  select(-`Universal Healthcare`, - Country, -`Tuition Fees in USD Thousands less 0`,-`% of World GDP`,-`Affordable Housing`, -`Mortgage as a % of Income`)
...
ggplot(wealth_share_long, aes(x=Year, y=Percent)) +
  ...
  labs(
    ...
    y = "Values"
           ...
           label = "Reaganomics Implementation", angle = 0, vjust = -15, hjust =-0.5, color  = "#2C5F8D")
ggplot(save_rate, aes(x=Year, y=`Personal Savings Rate`)) +
  ...
  annotate("text", x = 2020, y = max(country_data$Value) * 0.9, 
           label = "COVID-19", angle = 0, vjust = -25, hjust = 1.5, color = "#2C5F8D") +
...
##testing smi
```{r}
c.data.notest2<-c.data.notest2%>%select(-`Affordable Housing`)
lm_smi_test_noedu<-lm(`Social Mobility Index`~ . -`GDP Per Capita` -`Poverty Rate`, data = c.data.notest2)
summary(lm_smi_test)
```
...
relaxed_lasso_log_gdp <- lm(`logGDPPC` ~ `Corruption Perception` + `Education Index` + `Welfare Spending % GDP`, no_na.testloggdp)
...
create_plots <- function(model) {
  ...
  residual_plot <- ggplot(augmented_data, aes(x = .fitted, y = .resid)) +
    ...
    geom_smooth(method = "loess", se = FALSE, color = "#cfdee7") +
    ggtitle("Residuals vs Fitted Values") +
    xlab("Fitted Values") +
    ylab("Residuals")+theme(
    plot.background = element_rect(fill = "#FAFAFAFF", color = NA), # Light gray background
    panel.background = element_rect(fill = "#FAFAFAFF", color = "#2C5F8D", size = 1), # Dark blue border
    panel.grid.major = element_line(color = "#d0ddeb"), # Light gray grid lines for contrast
    panel.grid.minor = element_line(color = "#d0ddeb"), # Light gray grid lines for contrast
...
gdp_plots <-create_plots(lm_log_GDP)
